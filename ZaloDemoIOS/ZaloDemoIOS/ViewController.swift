...
var cuongSender = Sender(senderId: "1", displayName: "Duong Van Cuong")
var kienSender = Sender(senderId: "2", displayName: "Duong Van Kien")


class ViewController: MessagesViewController, MessagesDataSource, MessagesLayoutDelegate, MessagesDisplayDelegate, InputBarAccessoryViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, CLLocationManagerDelegate {
...
//            UserDefaults.standard.set(message, forKey: "chatMessagerList")
    ...
    @IBAction func btnNextUser(_ sender: UIBarButtonItem) {
            if isFisrtUser{
                isFisrtUser = false
                navigationItem.title = currentSender.displayName
                messagesCollectionView.reloadData()
